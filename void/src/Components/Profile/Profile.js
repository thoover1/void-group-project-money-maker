        ...
        <div className='profile-display'>
          {editImage 
            ? <div className='picture-holder'> 
                <div className='edit-holder'>
                  <input onChange={(e) => {this.toggle('img', e.target.value)}} placeholder='New Image' />
                  <span>
                    <button onClick={() => this.toggle('editImage', false)}>Cancel</button>
                    <button onClick={() => {img ? this.updatePic() : (window.alert('Please input a new image.')); this.toggle('editImage', false);}} >Save</button> 
                  </span>
                </div>
              </div>
              : <div className='picture-holder'> 
          ...
          {editName 
            ? <div className='username-holder'> 
                <input onChange={(e) => {this.toggle('username', e.target.value)}} placeholder='New Username' />
                <span>
                  <button onClick={() => this.toggle('editName', false)}>Cancel</button>
                  <button onClick={() => {username ? this.updateUsername() : (window.alert('Please enter a new username.')); this.toggle('editName', false);}} >Save</button> 
                </span>
              </div>
            : <div className='username-holder'> 
                <label className='username'>Username: {this.props.user.username}</label>
                <button onClick={() => this.toggle('editName', true)}>Edit</button> 
              </div>
          }
          {editEmail
            ? <div className='email-holder'> 
                <input onChange={(e) => {this.toggle('email', e.target.value)}} placeholder='New Email' />
                <span>
                  <button onClick={() => this.toggle('editEmail', false)}>Cancel</button>
                  <button onClick={() => {email ? this.updateEmail() : (window.alert('Please enter a new email.')); this.toggle('editEmail', false);}} >Save</button> 
                </span>
              </div>
            : <div className='email-holder'> <label className='email'>Email: {this.props.user.email}</label>
                <button onClick={() => this.toggle('editEmail', true)}>Edit</button> 
              </div>
          }
          {editPassword
            ? <div className='password-holder-edit'> 
                ...
                <span>
                  <button onClick={() => this.toggle('editPassword', false)}>Cancel</button>
                  <button onClick={() => {password ? this.updatePassword() : (window.alert('Please enter a new password.')); this.toggle('editName', false);}} >Save</button>
                </span>
              </div>
            : <div className='password-holder'> 
                <label className='password'>Change Password</label>
                <button onClick={() => this.toggle('editPassword', true)}>Edit</button> 
              </div>
          }
